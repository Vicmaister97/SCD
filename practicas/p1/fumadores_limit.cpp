...
const int tipos_ingred = 3, 					// número de tipos de ingredientes que hay, define cuántos fumadores hay
			cant_ingred = 40,					// cantidad que hay de cada ingrediente
			iter = cant_ingred * tipos_ingred;	// iteraciones del estanquero
int num_ingred[tipos_ingred] = {cant_ingred, cant_ingred, cant_ingred};	// array que almacena el número de ingredientes que quedan de cada tipo
...
void funcion_hebra_estanquero(  )
{
	int producto;
	int i = 0;
	while (i < iter){									// mientras queden ingredientes por producir
		...
		while (num_ingred[producto] == 0){				// si no quedan ingredientes de ese tipo
			producto = aleatorio<0, tipos_ingred-1>();	// produce un nuevo ingrediente
		}
		...
		num_ingred[producto]--;							// queda un ingrediente menos del producido
		i++;											// una iteración mas del estanquero
	}

	cout << "FIN estanquero, ha producido " << i << " ingredientes en total" << endl;
}
...
{
	int consumido = 0;
	while( consumido < cant_ingred )						// mientras queden de los ingredientes que consume
	{
		...
		consumido++;											// actualizamos el contador de cuántas veces ha fumado
	}

	cout << "FIN fumador " << num_fumador << ", ha fumado " << consumido << " veces" << endl;
}
