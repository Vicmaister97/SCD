...
int iterfum2 = 0;					// número de cigarros fumados por el fumador 2
...
{
	if (num_fumador == 2 && iterfum2 == 3){		// si el fumador 2 ya ha fumado 3 cigarros
		iterfum2 = 0;							// actualizamos el numero de cigarros fumados para empezar de nuevo el ciclo normal
		mtx.lock();
		cout << "                  Fumador " << num_fumador << " desecha su ingrediente (4 cigarro)" << endl << flush;
		mtx.unlock();
	}
		...
		// si se trata del fumador 2, actualizamos el número de cigarros fumados
		if (num_fumador == 2)
			iterfum2++;
	}
...
class Estanco : public HoareMonitor
{
 ...
 CondVar         // colas condicion:
	...
	esperafumador2,			 //  cola donde espera el fumador 2 en la iteración que no tiene que fumar (iterfum2==3)
...
void Estanco::obtenerIngrediente( int ingred )
{
	...
	while ( mostrador_vacio == true || ingrediente != ingred){	// bucle para asegurar que el fumador solo sale cuando se cumplen ambas condiciones
		if (ingred == 2 && iterfum2 == 3){		// si se trata del fumador 2 que ya ha fumado 3 cigarros
			mtx.lock();
			cout << "                  Fumador 2 espera en su cola especial" << endl << flush;
			mtx.unlock();
			esperafumador2.wait();		// espera bloqueada del fumador2 hasta que el estanquero coloca su ingrediente a desechar
		}
		else{
...
void Estanco::poner_ingrediente( int valor )
{
	...
	if (valor == 2 && iterfum2 == 3){		// si se trata del fumador 2 que ya ha fumado 3 cigarros
		esperafumador2.signal();			// avisa al fumador 2 de que puede coger su ingrediente que va a desechar
		return;
	}
	...
	for (i = 0; i < tipos_ingred; i++){
		if ( mostrador_vacio == true)		// si se ha notificado al indicado
			break;
		mtx.lock();
		cout << "Notificado vez " << i << endl << flush;
		mtx.unlock();
		ocupadas.signal();
	}
