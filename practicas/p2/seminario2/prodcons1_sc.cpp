...
class ProdCons1SCFIFO
{
 ...
 public:                    // constructor y métodos públicos
	ProdCons1SCFIFO(  ) ;           // constructor
	int  leer();                // extraer un valor (sentencia L) (consumidor)
	void escribir( int valor ); // insertar un valor (sentencia E) (productor)
} ;
// -----------------------------------------------------------------------------

ProdCons1SCFIFO::ProdCons1SCFIFO(  )
{
...
void ProdCons1SCFIFO::escribir( int valor )
{
	// ganar la exclusión mutua del monitor con una guarda
	unique_lock<mutex> guarda( cerrojo_monitor );
...
void funcion_hebra_productoraFIFO( ProdCons1SCFIFO * monitor )
{
	for( unsigned i = 0 ; i < num_items ; i++ )
	{
		int valor = producir_dato() ;
...
void funcion_hebra_consumidoraFIFO( ProdCons1SCFIFO * monitor )
{
	for( unsigned i = 0 ; i < num_items ; i++ )
	{
		int valor = monitor->leer();
...
int main()
{
	...
	ProdCons1SCFIFO monitor ;

	thread hebra_productora ( funcion_hebra_productoraFIFO, &monitor ),
			 hebra_consumidora( funcion_hebra_consumidoraFIFO, &monitor );
